...
class ExpParams:
    def __init__(self, pi_b, env, n_actions, n_states, exp_cost, gamma, max_depth, P, mask, n_sims):
...
class ActionNode:
    ...
    def __init__(self, action, state, rollout=True):
        ...
        self.visited = dict()
        for ki in exp_params.P[str(state)][str(action)].keys():
            self.visited[ki] = 0
    ...
    def simulate(self, state, depth, k):
        ...
        if (sum_p <= 0 + 0.00001) or depth > exp_params.max_depth:
            ...
            # reward = exp_params.R[state][self.action]
            reward = exp_params.env.get_instantaneous_reward(state)
        ...
        # actual_reward = exp_params.R[state][self.action]
        actual_reward = exp_params.env.get_instantaneous_reward(state)
...
class StateNode:
    def __init__(self, state=0, initial_n=0, initial_val=0, root=False):
        ...
        self.pi_b_masked = exp_params.pi_b[state]
        self.pi_b_masked[exp_params.mask[str(state)]] = 0
        ...
        self.non_safe_actions = np.where(~exp_params.mask[str(state)])
        new_nonsafe = []
        for i in self.non_safe_actions[0]:
            if i in exp_params.P[str(state)]:
                new_nonsafe.append(i)
        self.non_safe_actions = np.array(new_nonsafe)

        self.safe_actions = np.where(exp_params.mask[str(state)])
        new_safe = []
        ...
        self.ks = dict()
        for a in exp_params.P[str(self.state)].keys():
            if exp_params.mask[str(state)][int(a)]:  # safe
                self.ks[int(a)] = self.p_safe
            else:
                self.ks[int(a)] = self.pi_b_masked[int(a)]
        ...
        # changed here UCT
        self.ns = dict()
        for i in exp_params.P[str(state)].keys():
...
class StateNodeNew(StateNode):
    ...
    def stats(self):
        """Return the states: q-values, number of visits, value, old value (only UCT, no our formula)"""
        q_val = np.zeros(exp_params.n_actions)
        oa = collections.OrderedDict(sorted(self.actions.items()))
        for a in oa.keys():
            q_val[a] = oa[a].value()

        total_ns = np.zeros(exp_params.n_actions)
    ...
    def create_children_all(self, state):
        """Expand all actions"""
        for a in exp_params.P[str(state)].keys():
            self.actions[int(a)] = ActionNode(int(a), state)
    ...
    def rollout(self, state, depth):
        ...
        pi_b_masked = exp_params.pi_b[state]
        ...
        if (sum_p <= 0 + 0.00001) or depth > exp_params.max_depth:
            reward = k * exp_params.env.get_instantaneous_reward(state)
            return reward
    ...
    def pick_action(self, custom_prob, state=None):
        ...
        pi_b_masked = exp_params.pi_b[state]
        pi_b_masked[exp_params.mask[str(state)]] = 0
...
class MCTSSPIBB:
    ...
    def __init__(self, env, pi_b, nb_states, nb_actions, MLE_T, mask, states_to_sim, initial_policy, initial_qvalues, gamma,
                     n_sims=100, max_depth=15, exploration_costant=10,):
    ...
    def computeTree(self, s):
        ...
        global exp_params
        exp_params = ExpParams(self.pi_b, self.env, self.nb_actions, self.nb_states, self.exploration_costant, self.gamma,
                               self.max_depth, self.P, self.mask, self.n_sims)
    ...
    def fit(self):
        ...
        results = dict()
        print(len(state_sims))
        for s in state_sims:
            print(s)
            results[s] = self.computeTree(int(s))
